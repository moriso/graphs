...
void MainWindow::createAlgoritm ( int i ) {
    ...
    case ORDENACAO : {
        ...
        connect( ord,  SIGNAL ( update(Graph*)), this, SLOT ( mostrarGraph(Graph* ) ) );
        this->ord->start();
        break;
    }
    ...
    if (i != ORDENACAO){
        connect( alg,  SIGNAL ( update(Graph*)), this, SLOT ( mostrarGraph(Graph* ) ) );
        alg->start();
    }
}
...
void MainWindow::mostrarGraph ( Graph *g ) {
    this->tmp = g;
    update ();
}
