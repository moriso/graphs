...
Interface::Interface(QWidget *parent) :
    ...
    qDebug() << geo;
    this->grafo=new Graph(0, this);
    this->tmp=NULL;

    connect( this, SIGNAL (mostrar( Graph* )), this, SLOT ( mostrarGraph(Graph* ) ) );
...
void Interface::mostrarLista(QList<Vertex *> lista){
    qDebug() << "mostrarLista";
    QString str = "";
...
void Interface::getCaminho(QString metodo){
    Vertex * vertice = this->grafo->getVertex()[interface->cbFinal->currentIndex()];
    QString msg = "";
    int cont = 0;
    ...
    msg = this->grafo->getVertex()[interface->cbOrigem->currentIndex()]->getNome() + msg;
    qDebug() << msg;
...
void Interface::executarAlgoritmo ( int i ) {
    ...
    if (i != ORDENACAO){
        connect( algoritmo,  SIGNAL ( update(Graph*)), this, SLOT ( mostrarGraph(Graph* ) ) );
        algoritmo->start();
    }
}
...
void Interface::mostrarGraph ( Graph *grafo ) {
    this->tmp = grafo;
    update ();
}
